...
def create_insert_query(table_name, columns, primary_keys):
    # NOTE: 무료 버전에서는 DML(INSERT/UPDATE/DELETE/MERGE) 쿼리를 사용할 수 없습니다.
    ...
    WHEN MATCHED THEN 
        UPDATE SET {', '.join([f'T.{col} = S.{col}' for col in non_pk_columns])}
    WHEN NOT MATCHED THEN 
        INSERT ({', '.join(columns)}) VALUES ({', '.join([f'S.{col}' for col in columns])});
    """
    return merge_query
...
def create_job_config(data: list):
    job_config = bigquery.QueryJobConfig(
        query_parameters=[
            bigquery.ScalarQueryParameter(title, content_format, content) for title, content_format, content in data
        ]
    )
    return job_config
